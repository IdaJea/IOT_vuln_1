...
UTT ER820G devices have an authentication bypass and command injection vulnerability in the `sub_446A5C` function, which allows remote attackers to execute arbitrary commands via parameter `hangstr` passed to the binary through a POST request.
...
2. **command injection:** In the `sub_446A5C` function, user-supplied data is obtained from the HTTP request parameter `dialstr` through the `websGetVar` function and stored in the `Var` variable. The `Var` is then directly passed to several `doSystem()` calls that execute shell commands, thus causing a command injection vulnerability.

![image-20260128163518444](C:\Users\XiaoA\AppData\Roaming\Typora\typora-user-images\image-20260128163518444.png)
