...
After authentication bypass (or with a valid authenticated session), an attacker can trigger a stack buffer overflow in `compare_parentcontrol_time`. The handler parses the `time` parameter using unbounded `sscanf` into fixed-size stack buffers.
