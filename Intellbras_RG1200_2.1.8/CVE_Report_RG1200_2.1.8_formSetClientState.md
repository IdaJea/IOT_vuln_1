...
After authentication bypass (or with a valid authenticated session), an attacker can trigger a stack buffer overflow in `formSetClientState`. When `limitEn` is enabled, multiple user-controlled parameters are formatted into a fixed-size stack buffer using `sprintf` without bounds checking.
