...
After authentication bypass (or with a valid authenticated session), an attacker can trigger a stack buffer overflow in `fromAddressNat`. The handler formats user-controlled strings into fixed-size stack buffers using `sprintf` without bounds checking.
