...
After authentication bypass (or with a valid authenticated session), an attacker can trigger a stack buffer overflow in `fromSetSysTime_sync`. The handler processes the user-controlled `timeZone` value and either parses it via unbounded `sscanf` or copies it via `strcpy` into fixed-size stack buffers.
...
2. **stack overflow**: If the `timeZone` value contains `:`, `fromSetSysTime_sync` executes `sscanf(timezone, "%[^:]:%s", timezone_buf, timezone_min_buf)` into fixed-size stack buffers (`char[16]` each) without width limits. Otherwise it uses `strcpy(timezone_buf, timezone)` into a fixed-size stack buffer (`char[16]`) without validating length.
